...
function startup(cfg) {
    ...
    return new Promise((ok,nok) => {
        ...
        console.log('Adding a new route domain=%s pattern=%s', cfg.domain, cfg.patter);
        console.log('My webhook URL is %s', myHookURL);
        client.inbound.addRoute({domain: cfg.domain,
            pattern: cfg.pattern, url : myHookURL}, ok, nok);
    });
}
